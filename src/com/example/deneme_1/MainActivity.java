...
public class MainActivity extends BaseGameActivity implements SensorEventListener  {
	...
	/*Yazý için kullanýlacak nesneler...*/
	private Font font;
	...
	public void onLoadResources()  {
		...
		//0, 0 parametreli frame'in baþlama kordinatlarý.
	...
	public Scene onLoadScene() {
		...
		//sceneMenu oluþturulur ve menu tamamlanýr.
		this.sceneMenu = new Scene();
		this.scene = new Scene();
		
		createMenu(); //Menu oluþturulur.
        createGameScene(); //Oyun scenei oluþturulur.
		...
		/*Deneme Alaný Baþlangýç | Silinecek!     /*
		denemeSprite = new Sprite(0, 100, textRegResim1);
		denemeSprite.setSize(222, 20);
		scene.attachChild(denemeSprite);
		/*Deneme Alaný Bitiþ | Silinecek!*/
		
        return sceneMenu; //Ýlk olarak sceneMenu dönmeli!
	}
	...
	/*Sadece karakteri çizmez, jumping, gravity, velocity burada ayarlanýr!*/
	...
	{
		float tempY, sizeW; //_ olanlar sað taraftaki blocklar için olutþuruldu.
		...
		{	
			if(i%2 == 0)
			{
				...
				{
					scorInc(); //Skor arttýrma iþlemi yapýlýr.
					tempY = 0;
	...
	{
		//Burda oyunSonu adlý fonksiyonu çaðýr ve onun içinde gameOver scenini göster.
	...
	public void createGameScene()
	{
        ...
        this.scene.setOnSceneTouchListener(new IOnSceneTouchListener() {
			...
			public boolean onSceneTouchEvent(Scene pScene, TouchEvent pSceneTouchEvent) {
				...
				/*Arka arkaya basýldýðýnda yükseliyormuþ þekli verilmesi için...*/
				if(speedY < -9.0f)
					animSprite.stopAnimation();
				else
				...
				continueGame = true;
				//Bu koþul sürekli basýmý engeller. (Basýlý tutmayý)
				if(pSceneTouchEvent.isActionDown()) {
					
					//if(pSceneTouchEvent.getX() > CAMERA_WIDTH - spriteSize * 2 || sensorX <= -2) //Dokunarak zýplama için
					if(sensorX <= -2)
					{
						if(flySoundRight != null)
							flySoundRight.play();
	...
	public void createMenu()
	{
		...
		spriteMenuAbout = new Sprite(0, 540, textRegMenuAbout); //Týklanma kýsmýný daha sonra yap!
